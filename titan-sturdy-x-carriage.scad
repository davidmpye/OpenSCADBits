...
//FIXME ADJUST THIS TO MOVE THEM.//they are 62 apart
mounting_holes = [[-25, 0], [37, 0], /*[57, 7]*/];
                            ...
                            difference() {
                                ...
                                translate([0, 0, top_thickness])
                                    linear_extrude(height = rim_thickness, center = true, convexity = 5)
                                        difference() {
                                            inner_base_shape();
                                            translate([-base_offset + 6, -hole_offset])
                                            //+10 was my fudge // DMP
                                                rounded_square(hole + 12 + 2 * wall, hole_width + 2 * wall, corner_radius + wall);

translate([-base_offset +41.5 , -20]) square([3,59]);
                ...
                //TMP ones
                translate([51.5 - base_offset, 15, nut_trap_thickness -5 - top_thickness/2]) 
                     cylinder(r = 4.5, h = nut_trap_thickness - top_thickness, center = true);
            ...
            translate([-base_offset, 0, 0]) {
                // hol to clear the hot end
                //10 was 0! DMP
                //added +20 to hole also..!
                translate([6, - hole_offset])
                //hole was 36, going to add 12
                    rounded_rectangle([hole + 12, hole_width, 2 * rim_thickness], corner_radius);

                translate([53.5, 0]) rounded_rectangle([18, 20], 0);
                translate([51.5, 15, nut_trap_thickness -top_thickness]) 
                    nut_trap(M3_clearance_radius, M3_nut_radius, M3_nut_trap_depth);
